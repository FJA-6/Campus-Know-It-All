...
![Spring AI](https://img.shields.io/badge/Spring_AI-2.0--M2-0EA5E9)
...
**完整启动步骤（环境、MySQL、Milvus / simple-vector、环境变量、排错）见 [STARTUP.md](./STARTUP.md)。**

**GitHub：** [https://github.com/FJA-6/miniRag](https://github.com/FJA-6/miniRag)

```bash
git clone https://github.com/FJA-6/miniRag.git
```
...
- Spring Boot `4.0.x`
- Spring AI `2.0.0-M2`
- Milvus（向量数据库，可选；可用 `simple-vector` 内存模式）
- DashScope（百炼）等兼容 OpenAI 的聊天与 Embedding 接口
- Rerank（见 `app.rag.rerank-endpoint`）
- Apache Tika `2.9.2`
...
详见 **[STARTUP.md](./STARTUP.md)**。摘要如下：

1. 准备 **JDK 17+**、**MySQL**（建库 `tinyrag`），可选 **Milvus** 或使用 Profile **`simple-vector`**。
2. 设置环境变量（至少 **`MYSQL_PASSWORD`**、**`SILICONFLOW_API_KEY`**，变量名以 `application.yaml` 为准）。
3. 在仓库根目录执行：
...
浏览器访问：**http://localhost:8080/**（内置上传与流式问答界面）。
...
```text
src/main/java/com/fja/ai/tinyrag
├── config
